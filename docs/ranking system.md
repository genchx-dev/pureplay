OUR OFFICIAL RANK SYSTEM
Tier,Color Theme,Min XP,Max XP,Suggested Badge Icon
Wood,Dark Brown,0,999,Wooden log / Tree
Copper,Copper Orange,"1,000","4,999",Copper coin / Wire
Bronze,Bronze,"5,000","14,999",Laurel / Trophy
Iron,Metallic Gray,"15,000","39,999",Hammer / Sword
Silver,Bright Silver,"40,000","89,999",Star / Crown
Gold,Gold,"90,000","179,999",Crown / Coin
Diamond,Light Blue,"180,000","349,999",Diamond gem
Platinum,White-Silver,"350,000","599,999",Wings / Bar
Titanium,Dark Gray + Blue,"600,000","999,999",Titanium plate / Robot
Ruby,Deep Red,"1,000,000+",∞,Ruby gem / Flame

This is strong because:

early ranks move fast,
higher ranks become difficult,
elite ranks feel meaningful.
Recommended Universal XP Rules
Match XP
Result XP
Win +50 XP
Draw +25 XP
Loss +15 XP
Bonus XP
Win Streaks
Streak Bonus XP
3 Wins +20 XP
5 Wins +50 XP
10 Wins +120 XP
Tournament Placement XP
Placement XP
1st +1000 XP
2nd +700 XP
3rd +500 XP
Top 16 +250 XP
Top 64 +100 XP
Participation +30 XP
Example Progression
Casual Player

A player plays:

10 matches weekly
wins 4
draws 2
loses 4

XP calculation:

(4×50)+(2×25)+(4×15)=310

Weekly XP:

310 XP

Meaning:

reaching Bronze takes a few weeks,
which feels rewarding.
Competitive Player Example

A stronger player:

Result Count
Wins 8
Draws 1
Losses 1

XP:

(8×50)+(1×25)+(1×15)=440

Add:

streak bonuses,
tournament placement XP,

and they could earn:

700–1500 XP weekly.

That makes higher-skilled players climb faster naturally.

Estimated Time To Reach Each Rank
Casual Player (~300 XP/week)
Rank Estimated Time
Bronze ~3 weeks
Iron ~4 months
Silver ~1 year
Competitive Player (~1200 XP/week)
Rank Estimated Time
Bronze <1 week
Iron ~1 month
Silver ~3–5 months
Gold ~1 year
Why This Structure Works
Wood → Bronze

Fast progression.
Players feel immediate reward.

Bronze → Silver

Real commitment begins.

Gold+

Now prestige matters.

These become:

elite tournament players,
recognizable usernames,
creators/streamers later.
VERY IMPORTANT DESIGN DECISION

You should NEVER let XP alone determine matchmaking.

Instead:

Matchmaking Should Use:
hidden skill rating (MMR/ELO)
XP Should Represent:
progression,
dedication,
activity,
prestige.

Otherwise:

grinders become overranked,
matchmaking becomes unfair.
Smart Long-Term Feature

Later you can add:

Rank Unlock
Bronze Ranked tournaments
Silver Premium competitions
Gold Sponsored events
Diamond Verified status
Ruby Invitational tournaments
